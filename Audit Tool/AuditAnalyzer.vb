...
Public Class AuditAnalyzer
    ...
    Sub SortData()
        ...
        Dim Start, Row, Column, Offset, Index As Integer
        ...
        With Sheet
            ...
            Source = .Range("A1", .Cells(LastRow, LastColumn))
            DataArray = Source.Value
        End With
        For Row = 1 To UBound(DataArray, 1)
            'Specimen Check
            If Int32.TryParse(Left(DataArray(Row, 2), 1), Offset) Then
                Specimen(Index) = DataArray(Row, 2)
                bool = True
            ElseIf Int32.TryParse(Left(DataArray(Row, 1), 1), Offset) Then
            ...
            If bool Then
                ...
                For Column = 3 To UBound(DataArray, 2)
                    If DataArray(Row, Column) <> Nothing Then
                        If Int32.TryParse(Right(DataArray(Row, Column), 1), Offset) Then
                ...
                For Column = Start To UBound(DataArray, 2)
                    If DataArray(Row, Column) = "/" Then
                        Original_to_Destination(Index) = DataArray(Row - 1, Column - 1) & " to " & DataArray(Row, Column - 1)
                        Department(Index) = DataArray(Row, Column + 1)
